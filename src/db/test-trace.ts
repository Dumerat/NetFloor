...
async function runTest() {
  ...
  console.log(
    `   Port de départ : ${topology.nodes.wallOutlet.name} [${topology.ports.wallPort.label}] (${startPortId})`
  );
  ...
  console.log(
    `   Longueur totale de câble cumulée : ${result.totalCableLengthMeters} m (${result.totalCableLengthMm} mm)`
  );
  console.log(
    `   Terminaison sur équipement actif : ${result.isTerminatedAtActiveDevice ? "OUI" : "NON"}`
  );

  if (result.terminalNode) {
    console.log(
      `   Équipement terminal : ${result.terminalNode.name} (${result.terminalNode.type})`
    );
    console.log(
      `   Port d'arrivée : ${result.terminalNode.portLabel} [Mode: ${result.terminalNode.portMode}]`
    );
  }

  if (result.resolvedVlan) {
    console.log(
      `   VLAN Actif Résolu : VID ${result.resolvedVlan.vid} — "${result.resolvedVlan.name}"`
    );
  }
  ...
  result.hops.forEach((hop) => {
    const lenStr =
      hop.cableLengthMm > 0 ? ` [${hop.cableLengthMm / 1000}m ${hop.cableCategory ?? ""}]` : "";
    console.log(
  ...
  if (result.hops.length !== 4) {
    throw new Error(
      `Échec : Attendu 4 sauts (Origine + 3 transitions), reçu ${result.hops.length}`
    );
  }
  ...
  if (
    hop1.transitionType !== "CABLE" ||
    hop1.portDirection !== "REAR" ||
    hop1.cableLengthMm !== 44200
  ) {
    throw new Error(
      "Échec : Le saut 1 doit traverser le câble horizontal vers le port arrière du patch panel"
    );
  }
  ...
  if (
    hop3.transitionType !== "CABLE" ||
    hop3.nodeType !== "SWITCH" ||
    hop3.cableLengthMm !== 1500
  ) {
  ...
  if (result.resolvedVlan?.vid !== 20 || result.resolvedVlan.name !== "VLAN_CORP_DATA") {
    throw new Error(
      `Échec : VLAN attendu VID 20 (VLAN_CORP_DATA), reçu VID ${result.resolvedVlan?.vid}`
    );
  }
